...
__export(sftp_exports, {
  ...
  SFTP_PIPELINE_DEFAULT_CHUNK_BYTES: () => SFTP_PIPELINE_DEFAULT_CHUNK_BYTES,
  SFTP_PIPELINE_DEFAULT_MAX_IN_FLIGHT: () => SFTP_PIPELINE_DEFAULT_MAX_IN_FLIGHT,
  SFTP_PIPELINE_MAX_CHUNK_BYTES: () => SFTP_PIPELINE_MAX_CHUNK_BYTES,
  ...
  switch (privateKey.asymmetricKeyType) {
    case "ed25519": {
      ...
      return {
        ...
        publicKeyBlob,
        sign: (data) => (0, import_node_crypto3.sign)(null, import_node_buffer7.Buffer.from(data), privateKey),
        type: "publickey",
        username
      };
    }
    case "rsa": {
      ...
      return {
        algorithmName,
        publicKeyBlob,
        sign: (data) => (0, import_node_crypto3.sign)(hash, import_node_buffer7.Buffer.from(data), privateKey),
        type: "publickey",
        username
      };
    }
...
function encodeSshKexInitMessage(options) {
  const cookie = options.cookie === void 0 ? (0, import_node_crypto4.randomBytes)(KEXINIT_COOKIE_LENGTH) : import_node_buffer9.Buffer.from(options.cookie);
...
function encodeSshTransportPacket(payload, options = {}) {
  ...
  const padding = options.randomPadding === false ? import_node_buffer12.Buffer.alloc(paddingLength) : (0, import_node_crypto7.randomBytes)(paddingLength);
...
async function readFullSlot(sftp, handle, offset, length) {
  ...
  while (received < length) {
    ...
    collected = collected === void 0 ? data : import_node_buffer19.Buffer.concat([collected, data]);
