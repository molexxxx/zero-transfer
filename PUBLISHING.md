...
4. Runs `npm run packages:generate` — rewrites every `packages/<scope>/package.json` to match the new version and manifest deps. (The `dist/*` bundles are produced by `tsup` in step 2's `npm run build`.)
...
.github/workflows/release.yml
  ...
  • for each packages/*/, npm publish ...                              ← 12 self-contained scoped packages
        │
        ▼
...
> **Note:** Each scoped package is a self-contained bundle — consumers do **not** need to install `@zero-transfer/sdk` separately. **All scoped packages have zero runtime npm dependencies** (the native SSH stack used by `@zero-transfer/sftp` and `@zero-transfer/classic` ships in-bundle).
...
1. Add a new entry to [`scripts/scope-manifest.mjs`](./scripts/scope-manifest.mjs):
   ```js
   {
     ...
     deps: {},              // add npm deps here if needed, e.g. { "some-lib": "^1.0.0" }
     exports: ["createNewProviderFactory", "NewProviderOptions"],
     examples: ["new-example.ts"],
   }
   ```
2. Create `src/entries/newscope.ts` that re-exports from `./core` and your new provider source.
3. Add `"newscope"` to the scope array in `tsup.config.ts`.
4. Run `npm run build && npm run packages:generate && npm run docs:scopes` to scaffold `packages/newscope/dist/` and `docs/scopes/newscope.md`.
5. Register OIDC trusted publishing for `@zero-transfer/newscope` on npmjs.com (see above).
6. Update [`README.md`](./README.md) scoped-packages table.
7. Release as usual: `npm run release:patch`.
